...
def insert_block_pre_query(id, fk, type, heading=None, description=None, video=None, items=None, questions=None, object_key=None) -> str:
    return \
        f"INSERT INTO private.blocks (fk, type, heading, description, video, items, questions, object_key) VALUES ({fk}, '{type}', '{heading or ''}', '{description or ''}', '{video or '-'}', '{items or '{}'}', '{json.dumps(questions or [])}'::JSONB, '{object_key}') RETURNING *"
